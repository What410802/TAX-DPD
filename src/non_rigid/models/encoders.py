...
def utonia_geometry_sha256(scene: torch.Tensor) -> str:
    """Hash one or more exact TAX-DPD geometry tensors."""

    value = scene.detach().float().cpu().contiguous()
    # Dataset centering is performed independently by NumPy (precompute side)
    # and PyTorch (training side); their float32 reduction order can differ by
    # <1e-6 m in the 15x frame.  Quantize only the identity key, never the
    # features sent to Utonia, so equivalent exported geometry gets one cache
    # record without hiding larger changes.
    value64 = value.numpy().astype(np.float64)
    relative = value64 - value64[..., :1]
    # 1e-3 in the 15x model frame is 0.067 mm in world units, below the
    # exported point/voxel resolution while comfortably absorbing reduction
    # differences.  The feature values themselves remain unquantized.
    canonical = np.round(relative, decimals=3).astype(np.float32)
    digest = hashlib.sha256()
...
class CachedUtoniaSlotFeatureEncoder(nn.Module):
    """Load frozen Utonia slot features generated by the CUDA sidecar."""

    def __init__(self, hidden_size, model_cfg):
        super().__init__()
        manifest_value = getattr(model_cfg, "utonia_feature_manifest", None)
        if not manifest_value:
            raise ValueError(
                "point_encoder=utonia_cached requires model.utonia_feature_manifest"
            )
        path = Path(manifest_value).expanduser().resolve(strict=True)
        document = json.loads(path.read_text(encoding="utf-8"))
        if document.get("schema") != "taxdpd.utonia-static-feature-cache/0.1":
            raise ValueError("unsupported Utonia feature-cache manifest")
        if document.get("feature_width") != 576:
            raise ValueError("Utonia feature cache must have width 576")
        ...
        for record in records:
            if not isinstance(record, dict) or set(record) != {
                ...
                raise ValueError("invalid Utonia feature cache record")
    ...
    def frozen_features(self, x, sample_ids=None):
        ...
        for index, scene in enumerate(x):
            ...
            with np.load(path, allow_pickle=False) as archive:
                if frozenset(archive.files) != frozenset({"features"}):
                    raise ValueError("cached Utonia NPZ must contain only features")
                features = np.asarray(archive["features"])
            if (
                features.dtype != np.float32
                or features.shape != (slot_count, 576)
                or slot_count != scene.shape[-1]
                or not np.isfinite(features).all()
            ):
                raise ValueError("cached Utonia features have an invalid shape or dtype")
            outputs.append(torch.from_numpy(features.copy()).to(x.device).t())
...
class UtoniaJointFeatureEncoder(nn.Module):
    ...
    def __init__(self, in_channels, hidden_size, model_cfg, *, slot_encoder=None):
        ...
        if slot_encoder is not None:
            self.geometry_encoder = slot_encoder
        elif self.model_cfg.point_encoder == "utonia_cached":
            self.geometry_encoder = CachedUtoniaSlotFeatureEncoder(
                hidden_size, model_cfg
            )
        else:
            self.geometry_encoder = UtoniaSlotFeatureEncoder(hidden_size, model_cfg)
...
class DisjointFeatureEncoder(nn.Module):
    ...
    def __init__(self, in_channels, hidden_size, model_cfg):
        ...
        if self.model_cfg.point_encoder in {"utonia", "utonia_cached"}:
