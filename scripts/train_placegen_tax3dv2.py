...
def _batch(
    dataset: PlaceGenTaxDpdDataset,
    indices: int | list[int] | tuple[int, ...],
    device: torch.device,
) -> dict[str, Any]:
    """Load and collate one or more dataset items onto ``device``.

    The original pilot always passed one index, which made the effective
    training batch size one even though the dataset itself is batchable.  Keep
    the integer form for byte-for-byte compatible callers while allowing the
    runner to use a real mini-batch.  Only ``sample_id`` is non-tensor data used
    by the model (for the Utonia cache); it is therefore collated as a list.
    """
    ...
    if any(tuple(item) != keys for item in items[1:]):
        raise ValueError("dataset items must expose identical fields")
    result: dict[str, Any] = {}
    for key in keys:
        ...
        if isinstance(first, torch.Tensor):
            if any(not isinstance(value, torch.Tensor) for value in values):
                raise TypeError(f"dataset field {key!r} mixes tensor and non-tensor values")
            try:
                result[key] = torch.stack(values, dim=0).to(device)
            except RuntimeError as error:
                raise ValueError(f"dataset field {key!r} cannot be batched") from error
        elif key == "sample_id":
...
def _loss_for_batch(
    ...
    *,
    ...
    losses, _, _ = flow_matching_loss(
        module._velocity_model(module._model_kwargs(batch)),
        endpoint,
        source=source,
        time=time,
    )
...
# Kept as a small compatibility shim for downstream smoke scripts that imported
# the old private helper name.
def _loss_for_item(
    module: Any,
    item: dict[str, Any],
    *,
    ...
    return _loss_for_batch(module, item, seed=seed, mode=mode)
...
def train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    training_cfg.val_batch_size = int(args.validation_batch_size)
    ...
    for epoch in range(args.epochs):
        ...
        train_batches = _batched_indices(train_count, args.batch_size)
        random.Random(args.seed + epoch).shuffle(train_batches)
        for offset, batch_indices in enumerate(train_batches):
            batch = _batch(train_dataset, batch_indices, device)
            ...
            if not bool(torch.isfinite(loss)):
                raise FloatingPointError(
                    f"non-finite train loss at epoch={epoch}, batch={batch_indices}"
                )
            loss.backward()
            torch.nn.utils.clip_grad_norm_(module.parameters(), args.grad_clip_norm)
            optimizer.step()
            train_values.extend([float(loss.detach().cpu())] * len(batch_indices))
        module.eval()
        validation_values: list[float] = []
        with torch.no_grad():
            validation_batches = _batched_indices(
                validation_count, args.validation_batch_size
            )
            for offset, batch_indices in enumerate(validation_batches):
                batch = _batch(validation_dataset, batch_indices, device)
                value = _loss_for_batch(
                    module,
                    batch,
                    seed=args.seed + 900000 + epoch * 1000 + offset,
                    mode=args.mode,
                )
                validation_values.extend([float(value.detach().cpu())] * len(batch_indices))
    ...
    checkpoint_payload = {
        ...
        "training": {
            ...
            "optimizer_steps_per_epoch": len(_batched_indices(train_count, args.batch_size)),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="actual train mini-batch size; default preserves the pilot's B=1 behavior",
    )
    parser.add_argument(
        "--validation-batch-size",
        type=int,
        default=1,
        help="validation mini-batch size; default preserves the pilot's B=1 behavior",
    )
