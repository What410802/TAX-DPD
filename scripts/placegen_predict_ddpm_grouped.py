"""Run target-free reconstructed TAX3Dv2 DDPM on one PlaceGen split."""
...
REPORT_SCHEMA = "placegen.tax3dv2-ddpm-grouped-prediction-report/0.1"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    inference = load_native_inference_index(args.inference_manifest)
    device = torch.device(args.device)
    if device.type == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("requested CUDA prediction but CUDA is unavailable")
    loaded = load_ddpm_checkpoint(
        args.checkpoint,
        device,
    ...
    for sample_index, sample in enumerate(selected_samples):
        ...
        result = predict_fm_pose_candidates(
            ...
            # Candidate-0 stays identical if a later diagnostic asks for more
            # candidates.  Use the same per-sample stride as the FM runner.
    ...
    if len(records) != EXPECTED_SPLIT_COUNTS[args.split]:
        raise RuntimeError(f"DDPM prediction did not cover the complete {args.split} split")
    checkpoint_path = Path(args.checkpoint).expanduser().resolve(strict=True)
    model_cfg = cfg["model"]
    report = {
        "schema": REPORT_SCHEMA,
        "quality_claim": False,
        "quality_assessment": "inconclusive-until-fixed-index-evaluation",
        "model": DDPM_MODEL_CAPABILITY,
        "architecture": {
            "point_encoder": model_cfg["point_encoder"],
            "diffusion_steps": int(model_cfg["diff_train_steps"]),
            "diffusion_inference_steps": int(model_cfg["diff_inference_steps"]),
            "diff_noise_scale": float(model_cfg["diff_noise_scale"]),
